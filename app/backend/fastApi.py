...
# This imports the function you just wrote to save Q&A pairs to MongoDB.
...
# This creates a reliable path to your project's root 'app' directory.
# __file__ is the path to this file (fastApi.py).
# .resolve() makes it an absolute path.
# .parents[1] goes up one level from /backend to /app.
_app_dir = Path(__file__).resolve().parents[1]


# This tells FastAPI where to find your frontend's static files (JavaScript and CSS).
# It "mounts" the physical directory 'app/frontend/static' to the URL path '/static'.
app.mount("/static",
          StaticFiles(directory=str(_app_dir / "frontend" / "static")),
          name="static")

# This tells FastAPI where to find your HTML template files.
templates = Jinja2Templates(directory=str(_app_dir / "frontend" / "templates"))
...
async def serve_home(request: Request):
    ...
    # This renders the 'index.html' template and returns it as an HTML response.
